...
function ProfilePage({ route }) {
  ...
  const unFollow = () => {
    ...
    fetch(`http://192.168.42.144:5000/unfollow/${params3}`, {
      method: 'POST',
      body: formdata2
    })
      .then((response) => {
  ...
  const follow = () => {
    ...
    fetch(`http://192.168.42.144:5000/addFollower/${params1}`, {
      method: 'POST',
      body: formdata3
    })
  ...
  useEffect(() => {
    fetch(`http://192.168.42.144:5000/getProfile/${params3}`)
      .then((response) => {
        if (!response.ok) {
    ...
    fetch(`http://192.168.42.144:5000/checkFollow/${params1}`, {
      method: 'POST',
      body: formdata,
    })
    ...
    fetch(`http://192.168.42.144:5000/home/${params3}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
